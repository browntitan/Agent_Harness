...
tool_tags: inspect_document_structure, search_document_sections, read_indexed_doc
task_tags: sections, clauses, tables, sheets, structure
version: 1
enabled: true
description: Use document outlines, sections, clauses, sheets, and targeted source reads to guide retrieval.
...
Use `inspect_document_structure` to find available sections, clauses, sheets, or table-like chunks. Then use `search_document_sections` for targeted evidence. Use `read_indexed_doc` when a hit needs surrounding source context, especially for spreadsheets, adjacent table rows, or multi-sentence clauses.
