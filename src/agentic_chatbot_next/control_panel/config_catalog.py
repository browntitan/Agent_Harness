...
def build_config_catalog(agent_names: Sequence[str] | None = None) -> ConfigCatalog:
    fields: List[ConfigFieldSpec] = [
        ...
        ConfigFieldSpec("CONTROL_PANEL_SOURCE_ALLOWED_ROOTS", "Source Allowed Roots", "Bootstrap", "Comma-separated local roots the Knowledge Builder may scan for folder/repo sources.", getter=lambda s: ",".join(str(path) for path in getattr(s, "control_panel_source_allowed_roots", ()) or ()), readonly=True, reload_scope="read_only"),
        ...
        ConfigFieldSpec("RERANK_ENABLED", "Rerank Enabled", "Retrieval", "Enable cross-encoder reranking after deterministic candidate filtering.", getter=lambda s: getattr(s, "rerank_enabled", True), kind="bool"),
        ConfigFieldSpec("RERANK_PROVIDER", "Rerank Provider", "Retrieval", "Provider used for candidate reranking.", getter=lambda s: getattr(s, "rerank_provider", "ollama")),
        ConfigFieldSpec("RERANK_MODEL", "Rerank Model", "Retrieval", "Model used by the reranking provider.", getter=lambda s: getattr(s, "rerank_model", "")),
        ConfigFieldSpec("RERANK_TOP_N", "Rerank Top N", "Retrieval", "Number of prefiltered candidates to score with the reranker.", getter=lambda s: getattr(s, "rerank_top_n", 12), kind="int", min_value=1),
        ConfigFieldSpec("RERANK_TIMEOUT_SECONDS", "Rerank Timeout", "Retrieval", "Timeout for reranker model calls.", getter=lambda s: getattr(s, "rerank_timeout_seconds", 30), kind="int", min_value=1),
        ConfigFieldSpec("RERANK_FALLBACK_TO_HEURISTICS", "Rerank Fallback", "Retrieval", "Fall back to deterministic ranking if reranker output is unavailable or invalid.", getter=lambda s: getattr(s, "rerank_fallback_to_heuristics", True), kind="bool"),
        ...
        ConfigFieldSpec("DOCLING_ENABLED", "Docling Enabled", "Runtime", "Opt in to Docling-based DOCX/XLSX parsing before the stable local fallback loaders. Requires requirements-docling.txt or the app-docling Docker target.", getter=lambda s: getattr(s, "docling_enabled", False), kind="bool"),
        ConfigFieldSpec("OCR_ENABLED", "OCR Enabled", "Runtime", "Enable PaddleOCR-backed OCR for scanned PDFs and image files. Keep disabled for faster local ingest unless you need it.", getter=lambda s: getattr(s, "ocr_enabled", False), kind="bool"),
        ...
        ConfigFieldSpec("FRONTEND_EVENTS_ENABLED", "Frontend Events Enabled", "Frontend Events", "Enable server-gated runtime transparency events for chat clients.", getter=lambda s: getattr(s, "frontend_events_enabled", True), kind="bool"),
        ConfigFieldSpec("FRONTEND_EVENTS_SHOW_STATUS", "Show Status", "Frontend Events", "Emit generic phase and status updates to frontend clients.", getter=lambda s: getattr(s, "frontend_events_show_status", True), kind="bool"),
        ConfigFieldSpec("FRONTEND_EVENTS_SHOW_AGENTS", "Show Agents", "Frontend Events", "Emit agent selection, handoff, worker, planner, verifier, and route activity.", getter=lambda s: getattr(s, "frontend_events_show_agents", True), kind="bool"),
        ConfigFieldSpec("FRONTEND_EVENTS_SHOW_TOOLS", "Show Tools", "Frontend Events", "Emit tool lifecycle activity and safe tool payload previews.", getter=lambda s: getattr(s, "frontend_events_show_tools", True), kind="bool"),
        ConfigFieldSpec("FRONTEND_EVENTS_SHOW_PARALLEL_GROUPS", "Show Parallel Groups", "Frontend Events", "Emit grouped parallel or sequenced tool and worker activity.", getter=lambda s: getattr(s, "frontend_events_show_parallel_groups", True), kind="bool"),
        ConfigFieldSpec("FRONTEND_EVENTS_SHOW_GUIDANCE", "Show Guidance", "Frontend Events", "Emit safe metadata about prompt and guidance markdown loaded for an agent.", getter=lambda s: getattr(s, "frontend_events_show_guidance", True), kind="bool"),
        ConfigFieldSpec("FRONTEND_EVENTS_SHOW_SKILLS", "Show Skills", "Frontend Events", "Emit safe metadata about resolved skill-pack markdown loaded for an agent.", getter=lambda s: getattr(s, "frontend_events_show_skills", True), kind="bool"),
        ConfigFieldSpec("FRONTEND_EVENTS_SHOW_CONTEXT", "Show Context", "Frontend Events", "Emit safe section/token metadata for the context assembled before the model call.", getter=lambda s: getattr(s, "frontend_events_show_context", True), kind="bool"),
        ConfigFieldSpec("FRONTEND_EVENTS_SHOW_MEMORY_CONTEXT", "Show Memory Context", "Frontend Events", "Allow safe previews of retrieved memory/context. Keep disabled for conservative privacy.", getter=lambda s: getattr(s, "frontend_events_show_memory_context", False), kind="bool"),
        ConfigFieldSpec("FRONTEND_EVENTS_DETAIL_LEVEL", "Detail Level", "Frontend Events", "Frontend transparency detail level. Metadata shows counts and paths; safe previews also include clipped snippets.", getter=lambda s: getattr(s, "frontend_events_detail_level", "safe_preview"), kind="enum", choices=("metadata", "safe_preview")),
        ConfigFieldSpec("FRONTEND_EVENTS_PREVIEW_CHARS", "Preview Characters", "Frontend Events", "Maximum characters for each safe preview snippet.", getter=lambda s: getattr(s, "frontend_events_preview_chars", 480), kind="int", min_value=0),
        ...
        ConfigFieldSpec("CONTEXT_SMART_COMPACTION_ENABLED", "Smart Compaction Enabled", "Runtime", "Enable evidence-aware tool-result compaction for graph, RAG, and structured tool outputs.", getter=lambda s: getattr(s, "context_smart_compaction_enabled", True), kind="bool"),
        ConfigFieldSpec("CONTEXT_SMART_COMPACTION_LLM_ENABLED", "Smart Compaction LLM", "Runtime", "Allow a bounded LLM evidence-packing pass after semantic/rerank compaction.", getter=lambda s: getattr(s, "context_smart_compaction_llm_enabled", True), kind="bool"),
        ConfigFieldSpec("CONTEXT_SMART_COMPACTION_TARGET_TOKENS", "Smart Compaction Target", "Runtime", "Target model-visible token budget for compacted synthesis evidence.", getter=lambda s: getattr(s, "context_smart_compaction_target_tokens", 4000), kind="int", min_value=512),
